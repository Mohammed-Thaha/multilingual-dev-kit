...
const ProfileCard = ({ data, theme = 'en', isPreview = false }) => {
  const themes = {
  ...
  const currentTheme = themes[theme] || themes.en;

  return (
    <div 
      className={`profile-card ${isPreview ? 'preview' : 'translated'}`}
      style={{ 
        '--card-primary': currentTheme.primary,
        '--card-secondary': currentTheme.secondary 
      }}
      id={isPreview ? 'preview-card' : `card-${theme}`}
    >
      <div className="card-header">
        ...
        <div className="card-title">
          ...
          {!isPreview && <span className="language-badge">{currentTheme.name}</span>}
        </div>
      </div>

      <div className="card-content">
        <div className="card-headline">
          <h4>{data.headline || 'Your Headline'}</h4>
        </div>
        ...
        <div className="card-project">
          ...
          <p>{data.project || 'Your project description...'}</p>
        </div>
